...
void LCD_Init(void) {
    ...
    PORTDbits.RD1 = 0; LCD_Write(0x30,0);
    ...
    _delay((unsigned long)((5)*(20000000/4000.0)));
# 658 "./LCD_16x2.h"
    PORTDbits.RD1 = 0; LCD_Write(0x20,0);
    while (LCD_Busy()) {
        ;
    }
    while(LCD_Busy()); PORTDbits.RD1 = 0; LCD_Write(0x28,1);

    _displaycontrol = 0x04 | 0x00 | 0x00;
    while(LCD_Busy()); PORTDbits.RD1 = 0; LCD_Write((unsigned)0x08 | _displaycontrol,1);
    _displaymode = 0x02 | 0x00;
    while(LCD_Busy()); PORTDbits.RD1 = 0; LCD_Write((unsigned)0x04 | _displaymode,1);
    while(LCD_Busy()); PORTDbits.RD1 = 0; LCD_Write(0x01,1);
    while(LCD_Busy()); PORTDbits.RD1 = 0; LCD_Write(0x02,1);
}
...
void LCD_Write(char value, char mode) {
    ...
    PORTDbits.RD7 = LCD_data.bit7;
    PORTDbits.RD6 = LCD_data.bit6;
    PORTDbits.RD5 = LCD_data.bit5;
    PORTDbits.RD4 = LCD_data.bit4;

    PORTDbits.RD3 = 1; _delay((unsigned long)((1)*(20000000/4000000.0))); PORTDbits.RD3 = 0;;
    if (mode) {

        PORTDbits.RD7 = LCD_data.bit3;
        PORTDbits.RD6 = LCD_data.bit2;
        PORTDbits.RD5 = LCD_data.bit1;
        PORTDbits.RD4 = LCD_data.bit0;
        PORTDbits.RD3 = 1; _delay((unsigned long)((1)*(20000000/4000000.0))); PORTDbits.RD3 = 0;;
...
void LCD_Text(void *pt) {
    ...
    while (*text) {
        while(LCD_Busy()); PORTDbits.RD1 = 1; LCD_Write(*text,1);
        text++;
    }
}
...
void LCD_CustomChar(void *pt) {
    unsigned char *c = pt;
    for (char i = 0; i < 8; i++) {
        while(LCD_Busy()); PORTDbits.RD1 = 1; LCD_Write(*c,1);
        c++;
    }
}
...
void LCD_print_number(unsigned long num, char sign) {
    if (num == 0) {
        while(LCD_Busy()); PORTDbits.RD1 = 1; LCD_Write('0',1);
        return;
    }
    if (sign) {
        while(LCD_Busy()); PORTDbits.RD1 = 1; LCD_Write('-',1);
        num = ~num + 1;
    }
    ...
    while (lcd_div) {
        char c = num / lcd_div;
        while(LCD_Busy()); PORTDbits.RD1 = 1; LCD_Write('0' + c,1);
        num %= lcd_div;
        lcd_div /= 10;
    }
}
...
void main(void) {
    ...
    LCD_Init();
    while(LCD_Busy()); PORTDbits.RD1 = 0; LCD_Write(0x80 + (0x40 * 0) + 0,1);
    LCD_Text("Testeo....");
    while(LCD_Busy()); PORTDbits.RD1 = 0; LCD_Write(0x80 + (0x40 * 1) + 0,1);
    _displaycontrol |= 0x02; while(LCD_Busy()); PORTDbits.RD1 = 0; LCD_Write((unsigned)0x08 | _displaycontrol,1);
    _displaycontrol |= 0x01; while(LCD_Busy()); PORTDbits.RD1 = 0; LCD_Write((unsigned)0x08 | _displaycontrol,1);
    while(1){
